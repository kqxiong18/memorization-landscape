...
We further show that Personally Identifiable Information (PII) memorization operates at field granularity: virtually
...
information (PII)~\citep{carlini2021extractingtrainingdatalarge, lukas2023analyzing},
...
The research community has developed a rich set of measurement tools, including
exact match, $k$-eidetic recall, perplexity ratios, and
Min-K\%~Prob~\citep{shi2023detecting}, and attack methodologies, including
prefix extraction~\citep{carlini2021extractingtrainingdatalarge}, divergence
...
(Table~\ref{tab:cross-lit-summary}).
...
greedy extraction and the highest-loss quartile achieves 50.0\%, demonstrating
...
Table~\ref{tab:guidelines} summarizes protocols for three representative use
cases; these are principled implications of the FMARD decomposition, not
...
Table~\label{tab:cross-lit-summary} summarizes cross-literature mapping to FMARD links and failure modes.
...
($\tau_b > 0.63$ at all duplication levels, Table~\ref{tab:across-within-gap}).
