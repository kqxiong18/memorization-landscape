...
Figure~\ref{fig:corr-by-dup} shows within-cell $\tau$ by duplication level: even the best pair (MinK\%++ vs.\ Greedy EM) rises to $\tau \approx 0.70$ only at $d{=}256$, while Loss vs.\ Sampling EM swings to $-0.55$.
...
$\tau = 0.08$ under greedy extraction is a conservative lower bound, and that
...
$\tau(\Delta\text{Loss}, \Delta\text{EM}) = -0.256$ leaves substantial
...
EM variable and may partly inflate $\tau$ independently of the
...
Loss ($\tau = +0.070$, $p = 0.012$) and ZLib ($\tau = +0.084$, $p = 0.002$)
...
Notably, Min-K\% ($\tau = -0.039$, $p = 0.156$) and MinK\%++ ($\tau = -0.052$,
...
strongest for short sequences (Q1: $\tau = +0.148$, $n{=}150$) and near zero
for the second quartile (Q2: $\tau = +0.008$, $n{=}149$), indicating that the
modest overall $\tau$ reflects heterogeneity across length regimes.
...
EM on 600 WikiText-103 sequences using Pythia-6.9b-deduped.
Loss and ZLib correlate positively and significantly ($p < 0.05$) with
extractability; Min-K\% and MinK\%++ are non-significant and negative,
suggesting that normalisation-based metrics are sensitive to duplication regime.}
\label{tab:i13-tau}
\end{table}
...
\added{Two competing effects determine the net impact on M$\to$A $\tau$.
...
cooperative probing and collapsing $\tau$ toward zero in this regime.
...
and the observed $\tau$ values should be interpreted as M$\to$A correlation
...
The $\tau$ weakening therefore reflects a shift in metric calibration ---
...
which our $\tau = 0.08$ could change under a stronger adversary.}
...
is the expected direction, meaning our empirical $\tau = 0.08$ is a
\emph{conservative lower bound} on M$\to$A under adversarial conditions.}

\added{The implication for practitioners is asymmetric: a low observed $\tau$
...
Defenders who see $\tau \approx 0.08$ and conclude that loss is a poor
...
Directly measuring $\tau$ under soft-prompt optimisation on the HUBBLE
...
We find that Loss $\tau$ vs.\ Greedy EM is highest in the shortest sequence
quartile (Q1, $\leq 132$ tokens: $\tau = +0.148$) and near-zero in Q2
($132$--$177$ tokens: $\tau = +0.008$), a non-monotone pattern.
...
distribution and attenuating $\tau$.}
...
link: the 8B HUBBLE model exhibits $\tau$ sign inversion at high duplication
...
length normalisation to metric scores and should interpret near-zero $\tau$
...
However, the weak M$\to$A correlation ($\tau = 0.08$) implies that this
...
\added{The M$\to$A $\tau = 0.08$ implies a formal bound on what unlearning
...
probing, because $\tau = 0.08$ leaves 92\% of the Loss--EM variance
unaccounted for.
...
protection.
The empirical Kendall $\tau(\Delta\text{Loss},\,\Delta\text{EM}) = -0.256$
...
This is consistent with the weak M$\to$A upper bound ($\tau = 0.07$,
...
may show stronger $\tau(\Delta\text{Loss},\,\Delta\text{EM})$ or more reliable
per-sequence certification.
Third, $p$-values for $\tau = -0.256$ at $n = 50$ should not be corrected
