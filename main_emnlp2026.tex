...
\added{Appendix~\ref{app:unlearning-defense} analyses this gap analytically,
showing that the $\tau = 0.08$ bound implies metric-verified unlearning cannot
certify extraction-level protection without direct prefix-probing audits.}
...
\added{A controlled experiment directly testing this within HUBBLE would
fine-tune target models via gradient ascent on specific training sequences
(approximate unlearning), then measure $\Delta\text{Loss}$ and
$\Delta\text{Greedy EM}$ to estimate the empirical unlearning transfer rate.
Whether the $\Delta$Loss/$\Delta$EM ratio matches the M$\to$A $\tau = 0.08$
or diverges (due to redundant representations not captured by single-sequence
gradient ascent) is an open empirical question.
We leave this experiment to future work given the computational cost of
fine-tuning HUBBLE-scale models, but frame it here as the most direct test of
whether our M$\to$A estimate has actionable implications for unlearning
certification.}
