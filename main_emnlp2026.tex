...
\replaced{M$\to$A (Post-train)}{Post-training} & Unlearning     & No formal guarantee \\
\replaced{M$\to$A (Post-train)}{Post-training} & Alignment      & Masks, doesn't remove \\
...
\centering
\small
\begin{tabular}{@{}cp{3.0cm}p{5.8cm}@{}}
\toprule
...
\begin{figure}[H]
  \centering
  \includegraphics[width=\columnwidth]{figures/within_level_correlations.pdf}
...
Table~\ref{tab:across-within-gap} shows that pooling across duplication levels
inflates $\tau$; the mean gap (across $-$ within) is $+0.114$.
...
We complement the main analysis with a pipeline that stratifies simultaneously
by dataset and duplication level, quantifies the inflation introduced by
pooling, and includes repeated-query sampling alongside greedy extraction.

\paragraph{Setup.}
We evaluate the HUBBLE 8B perturbed model on 5{,}999 sequences spanning three
domains and six duplication levels.
Four forward-pass metrics (Loss, Min-K\%, MinK\%++, ZLib) are compared against
two extraction endpoints: static greedy and repeated-query sampling ($n{=}20$,
top-$k{=}40$, $T{=}1.0$).

\paragraph{The agreement ceiling is low.}
The highest mean within-cell $\bar{\tau}$ is 0.292 (MinK\%++ vs.\ Static
Greedy EM\@).
Loss vs.\ Static Greedy EM yields $\bar{\tau} = -0.251$
(Figure~\ref{fig:confounder-heatmap}).
...
paradox (Figure~\ref{fig:corr-by-dup}).
