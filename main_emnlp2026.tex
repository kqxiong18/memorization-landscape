...
% REVIEW: NeurIPS-C2/D2 - explicitly address the noise-attenuation alternative interpretation of τ = 0.08; the sign inversion at d=256 and below-chance tail overlap rule it out
% REVIEW: NeurIPS-D-new-1 - add footnote acknowledging Pearson disattenuation formula is an approximation for Kendall τ
...
derived value---the sign-inversion argument below is independent of it).
...
This remains low, but more importantly, the noise-attenuation account predicts
that the corrected correlation should be \emph{positive everywhere}---noise can
only dampen a true positive signal, not invert it.
The observed sign inversion at $d{=}256$ ($\tau = -0.52$ for Loss vs.\ Greedy
EM) directly falsifies this prediction: no amount of attenuation correction
can convert a negative observed correlation into a positive underlying signal.
The below-chance tail overlap at all duplication levels (cross-family top-10\%
agreement $= 0.077 <$ random baseline $0.10$) further confirms that the two
families do not merely measure the same variable at different precision levels.
