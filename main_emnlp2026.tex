...
Figure~\ref{fig:disagree-by-dup} shows this manifests as super-random threshold disagreement at $d{=}256$ ($76.8\%$ for Loss vs.\ Greedy EM, exceeding the $50\%$ random baseline).
...
Figure~\ref{fig:corr-by-dup} shows within-cell $\tau_b$ by duplication level: even the best pair (MinK\%++ vs.\ Greedy EM) rises to $\tau_b \approx 0.70$ only at $d{=}256$, while Loss vs.\ Sampling EM swings to $-0.55$.
...
Figure~\ref{fig:greedy-vs-sampling} further shows that greedy and probabilistic sampling attacks target different populations: at $d{=}16$, sampling discovers $1.7\times$ more extractable sequences than greedy decoding.
...
% Figure~\ref{fig:greedy-vs-sampling} moved to main body §3.5.
...
% Figure~\ref{fig:disagree-by-dup} moved to main body §3.3.
...
% Figure~\ref{fig:corr-by-dup} moved to main body §3.3.
