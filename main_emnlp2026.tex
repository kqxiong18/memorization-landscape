...
% ── Revision tracking: NO-OPS for clean submission ───────────────────────────
% \added{X}→X, \deleted{X}→(nothing), \replaced{X}{Y}→X
% The changes package is disabled; macros are no-ops so all tracked markup
% in the document body renders as final accepted text.
\newcommand{\added}[1]{#1}
\newcommand{\deleted}[1]{}
...
controls $Y$---and directional by construction: the upstream component shapes
...
measurement, measurement precedes exploitation).
...
The literature identifies seven primary factors (Table~\ref{tab:factors});
...
extraction performance (defined as zero greedy exact match on the forget set,
...
$\tau = 0.99$.
However, cross-family correlation averages $\tau_b = 0.08$
($p < 10^{-4}$, Kendall $\tau_b$ $z$-test; $\tau_b$ corrects for ties arising
from the binary exact-match outcome), indicating the two families rank
...
on that domain.
This also implies that metric choice is domain-sensitive: not all seven
...
levels; we note this value is consistent with a floor effect (greedy EM rates
...
We note that at $d{=}0$ (unduped insertions), high email extraction rates
...
While HUBBLE establishes that adversary capability modulates extraction
...
memorization, with a weak and Factor-dependent correlation ($\tau_b = 0.08$,
...
assigns low loss to fine-tuning data, shrinking the loss gap between members
and non-members (the signal MIA relies on) and reducing its discriminative
power~\citep{duan2024membership}.
