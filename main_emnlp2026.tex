...
% soul must load before microtype: the changes package loads soul internally,
% which redefines \showhyphens; if microtype loads first it CheckCommands the
% original definition and then soul's redefinition triggers a conflict error.
\usepackage{soul}
...
Cross-family correlation averages $\tau = 0.08$ but varies qualitatively by
content domain ($\tau \approx 0.035$ for literary text; $\tau \approx 0.33$
...
Factor configuration changes how well Metrics predict Attacks ($\tau_{M \to A}$
...
controls $Y$---and stronger than a mere correlation: the link encodes the
...
derived value).
With intra-family $\tau = 0.48$ for forward-pass metrics and $\tau = 0.99$ for
...
sequence-length effects are not yet dominant---cross-family $\tau$ differs
qualitatively by content type: $\tau \approx 0.035$ on literary prose
(Gutenberg) versus $\tau \approx 0.33$ on structured text (MMLU; see
\S\ref{sec:empirical}).
...
We note that the sign inversion at $d{=}256$ ($\tau = {-}0.52$ for Loss
vs.\ Greedy EM) is separately attributable to a sequence-length confound
(\S\ref{sec:empirical}): because length pushes Loss and extraction in opposite
...
which may partly inflate $\tau$ independently of the metric--attack
...
Copyright infringement
  & Loss $\tau \approx 0.035$ with extraction on literary text~(\S\ref{sec:empirical}); book-level variation~\citep{cooper2025extractingmemorizedpiecescopyrighted}
  & Extraction attacks on the content type of concern; loss-based pre-screening is insufficient \\
\addlinespace
PII leakage
  & Per-field $|\tau| < 0.12$ with document-level metrics; attack format swings risk by 88~pp~(\S\ref{sec:attack-reveal})
...
Regulatory compliance (unlearning)
  & Cross-family $\tau = 0.08$; metric-verified unlearning does not certify extraction protection~(\S\ref{app:unlearning-defense})
