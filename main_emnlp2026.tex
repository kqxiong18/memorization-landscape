...
% REVIEW: NeurIPS-C5/D - add operational definition of "directed dependency link" to prevent over-interpretation of the term
\added{Operationally, a \emph{directed dependency link} $X \to Y$ in FMARD
asserts that: (1) $X$ is a necessary precondition for $Y$ to have any signal
(removing $X$ makes $Y$ degenerate), and (2) properties of $X$ modulate the
strength or character of $Y$ (Factor configuration determines how well Metrics
predict Attacks).
This is weaker than a causal claim---we do not assert that intervening on $X$
controls $Y$---and stronger than a mere correlation: the link reflects the
logical structure of the memorization pipeline, independently of any
statistical association.
Prior SoK papers organize the same literature but do not trace this dependency
chain; Appendix~\ref{app:related} provides a structured comparison with
\citet{hartmann2023sok} and \citet{meeus2025sok} along five dimensions
(Table~\ref{tab:sok-comparison}).}
...
Covers F$\to$M$\to$A$\to$R$\to$D chain as linked components
  & Partial (definitions + implications, no chain)
  & No (parallel topics)
  & No (MIA-focused)
  & \textbf{Yes} \\
...
(Base: $0.029$; Aligned: $0.030$; $\Delta\text{EM} = +0.0008$), confirming
...
extraction probability, but does not erase memorised content or suppress the
M$\to$A link under greedy probing.
This is consistent with FMARD: RLHF inserts a Defense at the A$\to$output
interface, leaving the underlying M$\to$A relationship intact.}
