...
\footnote{This disattenuation formula is adapted from Pearson's correction for
attenuation~\citep{spearman1904proof}; it is an approximation when applied to
Kendall $\tau$, which does not share the same algebraic decomposition as
Pearson $r$.
We use the resulting estimate as an illustrative upper bound on the true-score
correlation, not a formally derived value.
The sign-inversion argument that follows is entirely independent of this
approximation.}
...
% REVIEW: NeurIPS-D-new-3 - hedge "does not reduce underlying extractability": sequences are in-distribution but not confirmed training members; conclusion scoped accordingly
Crucially, however, the mean greedy EM is nearly identical for both models
(Base: $0.029$; Aligned: $0.030$; $\Delta\text{EM} = +0.0008$).
\replaced{This indicates that RLHF alignment does not suppress in-distribution
greedy generation on this evaluation corpus.
Because the WikiText-103 sequences cannot be confirmed as training members of
\texttt{Mistral-7B-v0.1} (see dataset note above), this result is best
interpreted as: RLHF does not reduce greedy output probability on
...
training data is not publicly released.}{confirming
that RLHF alignment does not reduce underlying extractability.}
